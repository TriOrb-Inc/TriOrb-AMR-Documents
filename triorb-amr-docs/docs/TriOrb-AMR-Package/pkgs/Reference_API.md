...
[stella_vslam_ros のパッケージ一覧](Reference_API/stella-vslam-ros/index.md)
...
### [triorb_dead_reckoning](Reference_API/triorb-drive/triorb-dead-reckoning.md)

VSLAM・オドメトリ・IMUを統合し自己位置を推定するデッドレコニングパッケージです。IMUセンサ確認用バイパスログ機能も含みます。

### [triorb_drive_pico](Reference_API/triorb-drive/triorb-drive-pico.md)

ROS2メッセージを用いてモーター制御ECUと通信するためのパッケージ

### [triorb_drive_vector](Reference_API/triorb-drive/triorb-drive-vector.md)

制御指令値からロボットの進行方向や停止・回転などの状態判定を行う

### [triorb_navigation](Reference_API/triorb-drive/triorb-navigation.md)

自律移動を行うためのパッケージ

### [triorb_navigation_manager](Reference_API/triorb-drive/triorb-navigation-manager.md)
...
### [triorb_safe_run_cpp](Reference_API/triorb-drive/triorb-safe-run-cpp.md)

C++ implementation of the TriOrb safe run velocity filter.

### [triorb_snr_mux_driver](Reference_API/triorb-drive/triorb-snr-mux-driver.md)

SNR-MUXボードとシリアル通信し、音声再生状態・発進待ち時間などを ROS 2 トピックへ配信するドライバです。navigate / navigation_manager の停止・一時停止の遅延を音声再生と連動させます。

### [triorb_vslam_tf](Reference_API/triorb-drive/triorb-vslam-tf.md)
...
### [triorb_battery_info](Reference_API/triorb-os/triorb-battery-info.md)

CAN経由で受信したバッテリーSOC・各モジュール電圧/電流を集約し、/battery/status として配信するパッケージです。

### [triorb_gpio](Reference_API/triorb-os/triorb-gpio.md)

GPIOを通じてAMRの外部デバイス（ランプ・ブザー・トリガ等）を制御するためのノードを提供するパッケージです。

### [triorb_host_info](Reference_API/triorb-os/triorb-host-info.md)

ホストコンピューター（Jetson）関連の情報を表示するためのパッケージ

### [triorb_socket](Reference_API/triorb-os/triorb-socket.md)
...
### [sick_safetyscanners2_interfaces](Reference_API/triorb-sensor/sick-safetyscanners2-interfaces.md)

Interfaces for the sick_safetyscanners ros2 driver

### [sick_safetyscanners_base](Reference_API/triorb-sensor/sick-safetyscanners-base.md)
...
### [triorb_can](Reference_API/triorb-sensor/triorb-can.md)
